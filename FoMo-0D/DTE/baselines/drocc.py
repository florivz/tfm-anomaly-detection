...
class DROCCTrainer:
    ...
    def train(self, train_loader, learning_rate, lr_scheduler, total_epochs, 
        ...
        """Trains the prior on the given training dataset with periodic
    ...
    def test(self, test_loader):
        """Evaluate the prior on the given test dataset.
        Parameters
        ----------
    ...
    def one_class_adv_loss(self, x_train_data):
        ...
        3) Project the points between spheres of radius R and gamma * R 
            (set N_i(r))
        4) Pass the calculated adversarial points through the prior,
    ...
    def save(self, path):
        torch.save(self.model.state_dict(),os.path.join(path, 'prior.pt'))

    def load(self, path):
        self.model.load_state_dict(torch.load(os.path.join(path, 'prior.pt')))