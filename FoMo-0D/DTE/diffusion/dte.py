...
def binning(t, T=300, num_bins=30, device='cpu'):
    ...
    """
    return torch.maximum(torch.minimum(torch.floor(t * num_bins / T).to(device), torch.tensor(num_bins - 1).to(device)),
                         torch.tensor(0).to(device)).long()


class DTE():
    def __init__(self, seed=0, model_name="DTE", hidden_size=[256, 512, 256], epochs=400, batch_size=64, lr=1e-4,
                 weight_decay=5e-4, T=400, num_bins=7, device=None):
        ...
        def forward_noise(x_0, t, drift=False):
            ...
            sqrt_one_minus_alphas_cumprod_t = torch.take(sqrt_one_minus_alphas_cumprod, t.cpu()).to(
                self.device).unsqueeze(1)

            # mean + variance
            if drift:
                return (sqrt_alphas_cumprod_t.to(self.device) * x_0.to(
                    self.device) + sqrt_one_minus_alphas_cumprod_t.to(self.device) * noise.to(self.device)).to(
                    torch.float32)
            else:  # variance only
                return (x_0.to(self.device) + sqrt_one_minus_alphas_cumprod_t.to(self.device) * noise.to(
    ...
    def fit(self, X_train, y_train=None, X_test=None, y_test=None, verbose=False):
        ...
        train_loader = DataLoader(torch.from_numpy(X_train).float(), batch_size=self.batch_size, shuffle=True,
                                  drop_last=False)
...
class DTECategorical(DTE):
    def __init__(self, seed=0, model_name="DTE_categorical", hidden_size=[256, 512, 256], epochs=400, batch_size=64,
...
class DTEInverseGamma(DTE):
    def __init__(self, seed=0, model_name="DTE_inverse_gamma", hidden_size=[256, 512, 256], epochs=400, batch_size=64,
                 lr=1e-4, weight_decay=5e-4, T=400, device=None):
...
class DTEGaussian(DTE):
    def __init__(self, seed=0, model_name="DTE_gaussian", hidden_size=[256, 512, 256], epochs=400, batch_size=64,
                 lr=1e-4, weight_decay=5e-4, T=400, device=None):
        super().__init__(seed, model_name, hidden_size, epochs, batch_size, lr, weight_decay, T, 0, device)
