...
def binning(t, T, num_bins=30):
    return torch.maximum(torch.minimum(torch.floor(t * num_bins / T), torch.tensor(num_bins - 1)),
                         torch.tensor(0)).long()
...
class DTENonParametric(object):
    ...
    def plot_timestep_prediction(self, p_t=None, invgamma_p_t=None):
        ...
        ax.fill_between(self.T_range, invgamma_pred_mean - invgamma_pred_std, invgamma_pred_mean + invgamma_pred_std,
                        color="C3", alpha=0.2)
