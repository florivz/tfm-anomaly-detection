...
class PriorTrainDataGenerator:  # generate synthetic data for training
    ...
    def process_one_dataset(epoch_id, step, max_num_cluster, max_model_dim, every_n_dim, diversity, max_mean, max_var,
                            inflate_full, percentile, generate_fn, device):
        # Use a combination of epoch, step, and process ID to set the seed
        np.random.seed(epoch_id + step + os.getpid())
        torch.manual_seed(epoch_id + step + os.getpid())
        torch.cuda.manual_seed(epoch_id + step + os.getpid())
        ...
        # print('generating with dim', dim, 'num cluster', num_cluster)
        model = make_NdMclusterGMM(dim=dim, num_cluster=num_cluster, weights=torch.tensor([1 / num_cluster] * num_cluster, device=device),
                                   ...
                                   percentile=percentile, delta=0.05, device=device)
    ...
    def generate_batches(self, epoch, process_function=None, every_n_dim=10):
        ...
        for step in tqdm(range(total_tasks)):
            inliners, LA, sub_dims = process_function(
                epoch_id=epoch*total_tasks, step=step, max_num_cluster=self.max_num_cluster,
                max_model_dim=self.max_model_dim, every_n_dim=every_n_dim, diversity=self.diversity,
                max_mean=self.max_mean, max_var=self.max_var, inflate_full=self.inflate_full,
                percentile=self.percentile, generate_fn=self.generate_from_GMM, device=self.device)
            inliners_list.append(inliners)
            LA_list.append(LA)
            sub_dims_list.append(sub_dims)

        return inliners_list, LA_list, sub_dims_list
